...
public class dust extends Item {
	...
	public static String[] names = { "Chalky", "Enriched", "Pulsating",
			"Infused", "Darkened", "Mysterious", "Arcane", "Forged",
			"Enlightened" };
	...
	public boolean onItemUse(ItemStack par1ItemStack,
		...
		if (((dust) par1ItemStack.getItem()).i == 7) {
			return false;
		}
		...
		if (!par2EntityPlayer.canPlayerEdit(par4, par5, par6, par7,
			...
			if (par3World.canPlaceEntityOnSide(this.spawnID, par4, par5, par6,
				...
				if (par3World.setBlock(par4, par5, par6, this.spawnID, j1, 3)) {
					if (par3World.getBlockId(par4, par5, par6) == this.spawnID) {
						Block.blocksList[this.spawnID].onBlockPlacedBy(
								par3World, par4, par5, par6, par2EntityPlayer,
								par1ItemStack);
						Block.blocksList[this.spawnID].onPostBlockPlaced(
								par3World, par4, par5, par6, j1);
					}

					par3World.playSoundEffect(par4 + 0.5F, par5 + 0.5F,
							par6 + 0.5F, block.stepSound.getPlaceSound(),
